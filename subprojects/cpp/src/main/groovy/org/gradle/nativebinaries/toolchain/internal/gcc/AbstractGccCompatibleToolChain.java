...
public abstract class AbstractGccCompatibleToolChain extends AbstractToolChain implements PlatformConfigurableToolChain, ConfigurableToolChain {
    ...
    protected void registerTool(String name, ToolType type, String defaultExecutable) {
        toolRegistry.register(name, type, defaultExecutable);
    }
    ...
    protected void initTools(ToolChainAvailability availability) {
        ...
        if (!found) {
            availability.mustBeAvailable(locate((GccToolInternal)getByName("cCompiler")));
        }
    }
    ...
    public PlatformToolChain select(Platform targetPlatform) {
        ...
        //TODO Rene init dynamic registered tools
        initTools(result);
